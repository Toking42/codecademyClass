...
// Stuff needed to make Database Persistent
const yaml = require ("js-yaml");
const wyaml = require ("write-yaml");
const fs = require ("fs");

const DATABASE_PATH = 'data/database.yml';
...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.body && requestComment.articleId &&
    ...
    // Update Article and User with new Comment
...
function getComments(url, request) {
  ...
  response.status = 200;
  response.body = {
    comment: Object.keys(database.comment)
        .map(commentId => database.comment[commentId])
...
function upvoteComment(url, request) {
  if(DEBUG) console.log("-->Upvote Comment");
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const username = request.body && request.body.username;
  ...
  if(DEBUG) console.log("<--Upvote Comment: "+response.status);
  return response;
}

function downvoteComment(url, request) {
  if(DEBUG) console.log("-->Downvote Comment");
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const username = request.body && request.body.username;
  let savedComment = database.comments[id];
  const response = {};
  ...
  if(DEBUG) console.log("<--Downvote Comment: "+response.status);

  return response;
}
// Implement the YAML-Database-Persistence
function loadDatabase() {
  if(DEBUG) console.log('--> loadDatabase Function');
  let savedDatabase = yaml.safeLoad(fs.readFileSync(DATABASE_PATH, 'utf8'));
  if(DEBUG) console.log('<-- loadDatabase Function');
...
function saveDatabase() {
  if(DEBUG) console.log('--> saveDatabase Function');
  wyaml(DATABASE_PATH, database, function(err) {
      if(err) console.log('Error while writing database: '+err);
  })
  if(DEBUG) console.log('<-- saveDatabase Function');
}
...
if(DEBUG) console.log('--> Just before loadDatabase'+isTestMode);

if (typeof loadDatabase === 'function' && !isTestMode) {
  if(DEBUG) console.log('--> loadDatabase');
